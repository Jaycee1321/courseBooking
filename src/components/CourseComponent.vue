		...
		<div id="CourseCard" class="card p-2 cardHighlights shadow-sm" style="min-height: 100%">
			...
			<div class="card-body d-flex flex-column gap-2">
				...
				<p class="card-text text-muted mb-2">
					<!-- Limit the course description to 100 characters to prevent overflow -->
				...
				<div class="d-grid d-md-block mt-md-auto">
					...
					-->
						...
						<!-- <button v-if="isEnabled" class="btn btn-primary" v-on:click="enroll">Enroll</button>
						<button v-else class="btn btn-danger" disabled>Enroll</button>  -->
...
<script>
	...
	//import ref() from vue to create a reactive reference.
  	//import watch() from vue to use the watch() method.
    ...
    const notyf = new Notyf();
  	...
  	export default {
	    ...
	    //   const seats = ref(30);
	      ...
	      //Add a new reactive state using ref() for conditionally rendering our button.
	    ...
	    //   }
	      ...
	      //Use the watch() hook to track the changes made to a reactive reference/object and perform a task when the value changes.
          //watch() hook has two arguments, first is the reactive state to track and the second is the callback function that is run when the tracked state is is updated.
          //The callback function receives 2 values, the updated value of the state and its old value.
          //Every time the seats state is updated we will check its current value. If it is equal to 0, we will update the isEnabled ref/state to false which will disable the enroll button. Else, we will update the isEnabled value to true.
		...
		//Retrieve the global user state to use it as a condition for our ViewCourse button/link
